...
class BattleActions {
  ...
  canMegaEvo(pokemon) {
    ...
    // Atlas: Rayquaza must hold its mega stone. Upstream Showdown lets it mega
    // evolve on Dragon Ascent alone — we require the stone too. We can't rely
    // on item.megaEvolves / item.megaStone because the server's stone is a
    // custom item ("rayquazanite") that isn't in Showdown's items dex, so
    // getItem() returns an empty entry. Match the raw item id instead, and
    // accept both the canonical Showdown spelling ("rayquazite") and the
    // server-config spelling ("rayquazanite", from mega.yml's megastone_item).
    if (
      species.baseSpecies === "Rayquaza" &&
      pokemon.baseMoves.includes("dragonascent") &&
      (item.id === "rayquazanite" || item.id === "rayquazite")
    ) {
